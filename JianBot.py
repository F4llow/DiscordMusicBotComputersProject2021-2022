...
Rapi_key = "RGAPI-d60d311b-4a5f-433c-8b96-871c72bf4ad9"
watcher = LolWatcher(Rapi_key)
my_region = "na1"

@client.command()
async def info(ctx, *, summoner1):
    stats = watcher.summoner.by_name(my_region, summoner1)
    await ctx.send(stats)
    return print("the info command worked")
...
async def price(ctx, *, company):
    ...
    await ctx.send(company.get_price())
    return print("the price command worked")
